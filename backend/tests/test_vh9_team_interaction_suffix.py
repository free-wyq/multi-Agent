...
六段契约：
  ...
  D. 行为零变（system 层追加 + 决策层内嵌两层强化保持）
    9. registry sys_for_invoke 拼接 = ``system_prompt + "\n\n" + TEAM_INTERACTION_SUFFIX``
       （单聊不加——single_chat 分支保持原 persona 不变）。
   10. registry 仍是 ``if not self.single_chat:`` 守卫（单聊不加互动语义，B12 不改控制流）。
...
def assert_contract() -> list[str]:
    ...
    if not notify_body:
        ...
        # [10] 仍是 if not self.single_chat 守卫（单聊不加）
        if not re.search(r"if\s+not\s+self\.single_chat\s*:", notify_body):
            errs.append("[B10] sys_for_invoke 缺 if not self.single_chat 守卫（单聊应不加互动语义）")
        else:
            print("[B10] OK  sys_for_invoke 仍 if not self.single_chat 守卫（单聊不加，控制流不变）")
