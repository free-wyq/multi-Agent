...
B27 审计结论（4 维度现状对齐，行为零变不改码只补契约测锁防回归）：
  ...
  ── 重试：client.py 0 重试（max_retries 不接 httpx），由调用方包 try/except 兜底 ──
    chat_completion_stream / chat_completion 都**不**从 config 取 max_retries 喂 httpx
    （httpx.AsyncClient 的 transport 重试需 httpx.HTTPTransport(retries=)，本模块未配置）.
    client.py grep ``max_retries`` = 0 命中（与 agent_loop.py:196 ``chat_kwargs["max_retries"]``
    喂 langchain ChatOpenAI 的路径**不同**——那是 create_react_agent agentic loop 走 langchain，
    本模块直调 httpx 不经 langchain）. 故 client.py 的 LLM 调用失败**立即抛 RuntimeError**，
    不自动重试. 兜底在调用方：
      - coordinator _stream_coordinator_decision 被 node_llm_decide 的 try/except 包住
        （coordinator.py:889-904）→ RuntimeError 兜底成 ``decision = {action:chat, 兜底回复}``
        + ``_stream_stats = {}``（reply_id/tokens/elapsed_ms/model/reasoning_tokens 全空）.
      - worker _stream_brain_decision 被 node_brain_decide 的 try/except 包住
        （worker.py:281-306）→ RuntimeError 兜底成 ``decision = {action:chat, 兜底回复}``
        + ``_stream_stats = None``.
    两调用方兜底同形（decision 兜底 chat + stats 空值），非吞没（logger.warning 入日志）.
...
七段契约：
  ...
  B. 重试（client.py 0 重试，调用方 try/except 兜底）
    4. client.py 全文无 ``max_retries`` 喂 httpx（grep 0 命中——直调 httpx 不经 langchain）.
    5. coordinator _stream_coordinator_decision 被 node_llm_decide try/except 包（RuntimeError 兜底）.
    6. worker _stream_brain_decision 被 node_brain_decide try/except 包（RuntimeError 兜底）.
...
def assert_contract() -> list[str]:
    ...
    # ── B. 重试（client.py 0 重试）──
    # [4] client.py 全文无 max_retries 喂 httpx
    if "max_retries" in client:
        errs.append("[B4] client.py 含 max_retries（应为 0 重试——直调 httpx 不经 langchain）")
    else:
        print("[B4] OK  client.py 无 max_retries（直调 httpx 0 重试，与 agent_loop langchain 路径不同）")
    ...
    # [20] 两调用方 LLM 失败都兜底成 chat 回复（非抛）
    coord_chat_fallback = nld_body and '"content":' in nld_body and "理解有点困难" in nld_body
    worker_chat_fallback = nbd_body and '"content":' in nbd_body and "有点卡壳" in nbd_body
    if not (coord_chat_fallback and worker_chat_fallback):
        errs.append(f"[G20] 两调用方 LLM 失败未兜底成 chat 回复（coord={coord_chat_fallback} worker={worker_chat_fallback}）")
    else:
        print("[G20] OK  两调用方 LLM 失败兜底成 chat 回复（engine 不崩，降级对话继续）")
...
def main() -> int:
    ...
    print(
        ...
        "  · B 重试（client.py 0 重试直调 httpx 不经 langchain，调用方 try/except 兜底 RuntimeError）；\n"
